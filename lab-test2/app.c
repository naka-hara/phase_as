...
void main_task(intptr_t unused) {
	...
	while( act_flg ){
		...
		/* 超音波センサー取得 */
		distanceF = ev3_ultrasonic_sensor_get_distance( sonar_sensorF );
		if ( distanceF < 20 ) {
...
{
	int distanceF;	// 障害物とレーダの距離（cm）
//	int distanceB;	// 障害物とレーダの距離（cm）

	/* 距離の取得 */
	
	distanceF = ev3_ultrasonic_sensor_get_distance( sonar_sensorF );
//	distanceB = ev3_ultrasonic_sensor_get_distance( sonar_sensorB );
	
	if ( distanceF < 20 ) {
		
		sonar_error_flg = 1; // エラー
	}
	else {
		sonar_error_flg = 0;
	}
